      ...
      <div className="py-16 px-4">
          ...
          <div className="bg-gray-200 rounded-full p-1">
            <button
              className={`px-6 py-2 rounded-full ${
                billingCycle === 'quarterly' ? 'bg-orange text-white' : ''
              }`}
            ...
            <button
              className={`px-6 py-2 rounded-full ${
                billingCycle === 'annually' ? 'bg-orange text-white' : ''
              }`}
        ...
        <div className="flex justify-center items-center gap-0">
          {pricingData.map((plan) => (
            <div
              key={plan.type}
              className={`w-[381px] shadows ${
                plan.isPopular
                  ? 'bg-white shadow-xl w-[550px] h-[564px] rounded-xl z-10'
                  : 'bg-white h-[440px] self-center'
              }  overflow-hidden relative ${plan.isFirst ? 'rounded-l-lg' : ''} ${plan.isLast ? 'rounded-r-lg' : ''}`}
            >
              {plan.isPopular && (
              ...
              <div className="p-8">
                ...
                <div className={`text-[48px] ${ plan.isPopular ? 'text-[80px] text-dark-brown' : '' } text-[#332414] font-bold mb-6`}>
                  ₦{billingCycle === 'quarterly' 
                      ? plan.price.quarterly.toLocaleString()
                      : plan.price.annually.toLocaleString()}
                  <span className={`${ plan.isPopular ? 'text-[32px]' : '' } text-[24px] text-[#636363] font-normal`}>/{billingCycle === 'quarterly' ? 'Quarter' : 'Year'}</span>
                </div>
                <ul className="space-y-4 w-full mt-16">

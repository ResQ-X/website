  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const BREVO_LIST_ID = parseInt(
        process.env.NEXT_PUBLIC_BREVO_LIST_ID || "0"
      );
    ...
    <footer className="w-full overflow-x-hidden z-[10] bg-[#3B3835] text-white py-12 px-6 md:px-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
        <Image
          src={Logo}
          alt="ResQ-X Logo"
          className="w-[250px] h-[46.88px] object-contain"
          priority
        />
        <form
          onSubmit={handleSubmit}
          className="flex w-full h-[46px] max-w-[456px] md:w-auto"
        >
          <input
            type="email"
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-3">
              {pages.map((page) => (
                <li
                  key={page.name}
                  className="text-white font-[500] hover:text-orange transition-colors"
                >
        ...
        <div>
          ...
          <ul className="space-y-3">
            {partner.map((item) => (
              <li
                key={item.name}
                className="text-white hover:text-orange font-[500] transition-colors"
              >
